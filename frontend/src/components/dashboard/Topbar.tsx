...
function NavIcon({ kind }: { kind: "overview" | "portfolio" | "market" | "favorites" | "history" | "compare" }) {
  const paths = {
    ...
    favorites: "m12 20-1.45-1.32C5.4 14.36 2 11.28 2 7.5A4.5 4.5 0 0 1 6.5 3c1.74 0 3.41.81 4.5 2.09A6.02 6.02 0 0 1 15.5 3 4.5 4.5 0 0 1 20 7.5c0 3.78-3.4 6.86-8.55 11.18L12 20Z",
  ...
  return (
    <svg aria-hidden="true" className={styles.navIcon} fill="none" viewBox="0 0 24 24">
      <path d={paths[kind]} stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" />
    </svg>
  );
}
    ...
    <div className={styles.actions}>
      <span className={styles.userIdentity}>
        ...
        <span>
          ...
          <small>{isAutoRefreshEnabled ? "Sincronización activa" : "Vista manual"}</small>
        </span>
      </span>
      <button className={styles.iconButton} aria-label="Actualizar mercado" disabled={isRefreshing} onClick={() => void refresh()} type="button">
        <svg aria-hidden="true" fill="none" viewBox="0 0 24 24">
          <path d="M20 11a8 8 0 0 0-14.9-3M4 5v4h4m-4 4a8 8 0 0 0 14.9 3M20 19v-4h-4" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" />
        </svg>
        <span>{isRefreshing ? "Actualizando" : "Actualizar"}</span>
      </button>
      <button className={styles.logoutButton} onClick={logout} type="button">Salir</button>
    </div>
  );
}
